...
function draw(data) {
//  console.log("Updating canvas with pixels and drawing table");
  var c = document.getElementById("irCameraCanvas");
  var l = document.getElementById("irCameraTemps");
  ...
  var h = data.frame.length;
  var w = data.frame[0].length;

  c.height = (h + 2) * pixelSize;
  c.width = (w + 2) * pixelSize;
  l.height = 40;
  l.width = c.width;
  ...
  var html = data.timestamp + "<br/>&mdash;Degrees Celsius<br/><table>\n";

  var temps = data.frame;

  var maxTemp = getMax(temps);
  var minTemp = getMin(temps);

  ltx.clearRect(0, 0, t.width ,t.height);
  ltx.font = "Bold 18px Arial";
  var tempDif = maxTemp - minTemp;
  var legendInterval = (tempDif / 5).toFixed(1);
  var legendTemp = parseFloat(minTemp).toFixed(1);

  x = (l.width / 4);
  y = 40;

  for (i=0;i<6;i++) {
    ltx.fillStyle = "hsl(" + constrain(map(legendTemp, minTemp, maxTemp, 180, 360), 160, 360) + ", 100%, 50%)";
    var text = legendTemp + "\xB0  ";
    ltx.fillText(text, x, y);
    x += ltx.measureText(text).width;
    legendTemp = (parseFloat(legendTemp) + parseFloat(legendInterval)).toFixed(1);
  }

  for (y=0;y<temps.length;y++) {
    html += "<tr>\n";
    for (x=0;x<temps[y].length;x++) {


/* My hacky implementation of palette magic!
      // 8.8-44.8 C -- That's the magic? (-8.8 to get 0, /36 to get the delta between 8.8-44.8)

      var index = Math.floor(((temp - 8.8) / 36.0) * iron_palette.length);
      if (index < 0) {
        index = 0;
      } else if (index >= iron_palette.length) {
        index = iron_palette.length - 1;
      }

      var color = iron_palette[index]; */

      // paraphrased from github.com/sparkfun/.../MLXHeatCam.pde

      var color = "hsl(" + constrain(map(parseFloat(temps[y][x]), minTemp, maxTemp, 180, 360), 160, 360).toString() + ", 100%, 50%)";      
//      console.log("Color: ", color);
      ctx.fillStyle = color;
      ctx.fillRect((x + 1) * pixelSize, (y + 1) * pixelSize, pixelSize, pixelSize);

      html += '<td style="background-color: ' + color + '">' + parseFloat(data.frame[y][x]).toFixed(1) + '</td>';
...
define(function(require, exports, module) {
  ...
  var newView = SimpleSplunkView.extend({
      className: "irCameraView",
      outputMode: "json",
      options : {
        data: "preview"
      },
      ...
      createView: function() {
        this.$el.html('<canvas id="irCameraCanvas" width="320" height="240" style="filter: blur(12px);"></canvas><br /><canvas id="irCameraTemps" width="320" height="20"></canvas>');
      ...
      //  console.log("Formatting Data");
        ...
        //  console.log("Formatting data: ", data);
          ...
          formattedData = { 
            ...
            frame: JSON.parse(data[0]._raw).frame
          };
